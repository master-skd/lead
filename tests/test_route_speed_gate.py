...
def test_apply_gate_only_changes_speed_not_any_route_state():
    ...
    gated, factor = apply_collision_speed_gate(
        speed, torch.tensor([0.2, 0.7, 0.95]), config
    )
...
class _VelocityGateHead(nn.Module):
    def forward(self, route_features, current_speed, raw_target_speed, candidates):
        ...
        preference[:, 1] = 1.0
        risk = torch.full_like(preference, 0.1)
        risk[:, 0] = 0.9
...
def test_velocity_scorer_gate_changes_only_target_speed():
    from lead.inference.config_open_loop import OpenLoopConfig
    from lead.inference.open_loop_inference import OpenLoopInference
    from lead.tfv6.tfv6 import Prediction
    ...
    inference.config_open_loop = OpenLoopConfig(raise_error_on_missing_key=False)
    inference.device = torch.device("cpu")
    inference.velocity_scorer = _VelocityGateHead()
    inference.velocity_vocabulary = torch.tensor(
        [[-0.5, -1.0, -1.5, -2.0, -2.5, -3.0, -3.0, -3.0]]
    )

    logits = torch.full((1, len(config.target_speed_classes)), -10.0)
    logits[:, 2] = 10.0  # raw target is the 8 m/s class
    route = torch.arange(20, dtype=torch.float32).reshape(1, 10, 2)
    prediction = Prediction(
        pred_future_waypoints=None,
        pred_target_speed_distribution=logits,
    ...
    torch.testing.assert_close(result[0], route)
    torch.testing.assert_close(result[6], torch.tensor([[8.0]]))
    torch.testing.assert_close(result[2], torch.tensor([[0.0]]))
    torch.testing.assert_close(result[11], torch.tensor([0.9]))
