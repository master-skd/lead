#!/usr/bin/env bash
# B3a-0: no-training velocity oracle on the fixed confidence-selected route.
set -euo pipefail
...
# These commands are incremental: existing route splits and complete actor shards are reused.
...
"${LEAD_ENV}/bin/python" scripts/p4/eval_b3a_velocity_oracle.py \
    --ckpt-dir "${CKPT_DIR}" \
    ...
    --num-workers 4 \
    --limit "${LIMIT}" \
    --out "${OUTPUT_DIR}/velocity_oracle.json" \
    "$@"
...
if [[ "${B3A_SKIP_GPU_BURN:-0}" != "1" ]]; then
    ...
    cd "${GPU_BURN_DIR}"
    mkdir -p burn_logs
    GPU_BURN_LOG="${GPU_BURN_DIR}/burn_logs/b3a_velocity_oracle_$(date -u +%Y%m%dT%H%M%SZ).log"
    nohup ./gpu_burn -m 70% 99999999 >"${GPU_BURN_LOG}" 2>&1 < /dev/null &
    GPU_BURN_PID=$!
    echo "${GPU_BURN_PID}" > "${GPU_BURN_DIR}/burn_logs/b3a_velocity_oracle_latest.pid"
