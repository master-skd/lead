...
def _actors(position, yaw=0.0, valid=None, actor_id=7):
    ...
    positions = np.repeat(
        np.asarray(position, dtype=np.float32)[None, None], FUTURE_STEPS, axis=1
    )
...
def test_obb_collision_reports_first_time_and_actor():
    ...
    label = future_collision_label(
        ego, np.zeros(8), _actors([4.0, 0.0]), safety_margin_m=0.0
    )
...
def test_parser_samples_20hz_future_and_keeps_partial_mask():
    ...
    np.testing.assert_array_equal(
        frame.valid[0], [True, True, False, False, False, False, False, False]
    )
...
def test_route_interpolation_can_extrapolate_final_tangent():
    route = np.asarray([[2.0, 0.0], [5.0, 0.0], [5.0, 5.0]])
    clipped, _ = interpolate_route_by_distance(route, np.asarray([12.0]))
    extrapolated, yaw = interpolate_route_by_distance(
        route,
        np.asarray([12.0]),
        extrapolate=True,
    )
    np.testing.assert_allclose(clipped, [[5.0, 5.0]], atol=1e-6)
    np.testing.assert_allclose(extrapolated, [[5.0, 7.0]], atol=1e-6)
