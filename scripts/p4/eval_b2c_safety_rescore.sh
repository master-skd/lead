#!/usr/bin/env bash
# B2c open-loop safety-rescoring validation. No CARLA/Bench2Drive server is needed.
set -euo pipefail
...
"${LEAD_ENV}/bin/python" scripts/p4/eval_b2c_safety_rescore.py \
    --ckpt-dir "${CKPT_DIR}" \
...
# Keep the GPU(s) occupied in the background after evaluation finishes successfully.
cd "${GPU_BURN_DIR}"
mkdir -p burn_logs
GPU_BURN_LOG="${GPU_BURN_DIR}/burn_logs/b2c_safety_$(date -u +%Y%m%dT%H%M%SZ).log"
nohup ./gpu_burn -m 70% 99999999 >"${GPU_BURN_LOG}" 2>&1 < /dev/null &
GPU_BURN_PID=$!
echo "${GPU_BURN_PID}" > "${GPU_BURN_DIR}/burn_logs/b2c_safety_latest.pid"
